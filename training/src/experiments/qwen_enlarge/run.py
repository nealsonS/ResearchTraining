...
from ResearchTraining.util.detections import resize_bounding_boxes
...
def main():
    ...
    5. Calculate metrics
    """
    valid_image_paths = get_images_from_dir(IMAGE_DIR)
    RUN_CONFIG["valid_images"] = len(valid_image_paths)
    ...
    with mlflow.start_run():
        ...
        for img_path in tqdm(valid_image_paths):
            ...
            # enlarge box by X
            pred["boxes"] = resize_bounding_boxes(
                pred["boxes"], resize_by=RUN_CONFIG.get("enlarge_by", 1)
            )
